...
void mem_mngr_init(void) {
    ...
    batch -> batch_mem = calloc(new_mem_pool -> slot_size, new_mem_pool -> batch_count);
    batch -> next_batch = NULL;

    STRU_MEM_LIST* new_mem_pool = malloc(sizeof(STRU_MEM_LIST));
    new_mem_pool -> slot_size = 8;
    new_mem_pool -> batch_count = 1;

    // for allocating the bitmap
    

    new_mem_pool -> next_list = mem_pool;
    new_mem_pool -> first_batch = batch
